...
- [rewriteAppendOnlyFileBackground()](https://github.com/redis/redis/blob/7.0.0/src/aof.c#L2381)：创建后台子进程重写 AOF 文件
  ...
  - 父进程中，处理创建子进程失败的情况，并重置 `aof_rewrite_scheduled` 标志位
...
正如前面所提到的，可以通过设置 `aof_use_rdb_preamble` 标志位，在重写 AOF 文件时，按照 RDB 的格式进行重写。这种情况下，硬盘中的 AOF 文件的前半部分会是 RDB 格式的全量数据，磁盘占用小，加载速度快，后半部分是正常的 AOF 格式内容。
