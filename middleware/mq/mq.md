...
在实现上，**Exactly Once** 一般需要通过事务消息进行保障，方案较为复杂，且可能引发性能和可用性问题。
...
- **事务机制**：
  - 生产者事务：确保消息发送与本地事务原子性
  - 使用 `transactional.id` 标识事务，通过两阶段提交（2PC）保障
- **适用场景**：
  - 跨 Partition 的原子写入（例如 Exactly-Once 语义）
- **局限性**：
  - 事务消息与普通消息隔离，消费者需过滤未提交事务的消息
...
- **事务消息流程**
  - **半消息（Half Message）**：消息暂存到 Broker，但对消费者不可见
  - **本地事务执行**：生产者执行本地事务，返回 Commit/Rollback
  - **事务状态回查**：若生产者未提交状态，Broker 主动回查事务状态
  - **消息投递**：事务提交后，消息对消费者可见；回滚则丢弃
- **优势**
  - 解决分布式事务的最终一致性（如订单创建与扣款场景）
